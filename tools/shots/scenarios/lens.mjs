...
export default {
  ...
  async run(g){
    ...
    await g.eval(()=>{chapterReveal={index:1,age:0};});await g.advance(2.2);await g.shot('the-hague-ring');
    for(const [i,name] of [[2,'paris-glass'],[3,'meudon-ink'],[4,'tucson-channels'],[5,'canaveral-rendered']]){
      await g.eval(i=>{chapterReveal={index:i,age:0};},i);await g.advance(2.6);await g.shot(name);}
