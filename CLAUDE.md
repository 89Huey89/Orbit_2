...
Five pieces of that plan are already shipped, independent of the progression system
itself — Era I (`src/rock.js`), Era II (`src/ceiling.js`), Era III (`src/scroll.js`),
Era IV (`src/astrolabe.js`) and Era VI (`src/lens.js`), standalone preview doors on the frontispiece;
...
- `npm run test:quick` — the same file with `--quick`: skips the seeded playthroughs
  ...
  ledger). Under a minute instead of about a minute and a half on four cores. Use it for changes that leave gameplay alone. It
  ...
  tasks in `scripts/verify/tasks.mjs` themselves. CI always runs the full suite.
...
- `src/simulation.js` must stay **DOM-free**: the test suite extracts only the
  ...
  those markers in place, and update the destructuring lists in
  `scripts/verify/sandbox.mjs` if you rename or add to what it needs.
