...
// The endless driver (JOURNEY.md stage 4). A chart dealt with it is the chart dealt without it through
// row 28, it is nothing inside the first four minutes, and past both it keeps rising at rows 200 and 500
// on every channel it feeds rather than settling the way every curve before it did.
{
  const deal=(driven,rows)=>{const w=new OrbitWorld(77,440,860);w.driven=driven;w.keepAll=true;while(w.row<rows)w.generateRow();return w;};
  const flat=deal(false,560),driven=deal(true,560);
  const early=w=>JSON.stringify({nodes:w.nodes.filter(n=>n.row<=28),hazards:w.hazards.filter(h=>h.row<=28).map(({force,...h})=>h),nebulas:w.nebulas.filter(g=>g.row<=28)});
  assert.equal(early(driven),early(flat),'The driver deals the same chart as the flat one through row 28');
  assert(flat.hazards.every(h=>h.force===1),'A chart dealt without the driver blows every field at its old strength');
  const idle=new OrbitWorld(77,440,860);idle.driven=true;
  for(const [row,seconds] of [[0,0],[28,0],[0,230],[28,234]]){idle.progress=row;idle.elapsed=seconds;assert.equal(idle.difficultyDriver(),0,'The driver is nothing at row '+row+' and '+seconds+' s');}
  const at=(row,seconds)=>{idle.progress=row;idle.elapsed=seconds;return idle.difficultyDriver();};
  assert(at(500,0)>at(200,0)+.5&&at(200,0)>at(48,0),'The driver is still rising with the row at rows 200 and 500');
  assert(at(0,1800)>at(0,900)+.5&&at(0,900)>at(0,300),'The driver is still rising with the clock at fifteen and thirty minutes');
  const unDriven=new OrbitWorld(77,440,860);unDriven.progress=500;unDriven.elapsed=1800;
  assert.equal(unDriven.difficultyDriver(),0,'A world without the driver never feels it');
  assert(driven.pace(500)>driven.pace(200)&&driven.pace(200)>driven.pace(48)&&driven.pace(48)>driven.pace(28),'The pace a crossing is cut for keeps rising past row 28');
  assert.equal(flat.pace(500),flat.pace(28),'The flat chart settles its pace at row 28 as it always did');
  const tide=(w,row,seconds)=>{w.progress=row;w.elapsed=seconds;w.perfectStreak=0;return w.darknessSpeed();};
  assert.equal(tide(idle,28,234),tide(unDriven,28,234),'The flood is the flat one until four minutes in');
  assert(tide(idle,28,1800)>tide(idle,28,900)&&tide(idle,28,900)>tide(idle,28,400)&&tide(idle,28,400)>tide(idle,28,234),'The flood keeps rising with the clock past four minutes');
  assert(tide(idle,500,600)>tide(idle,200,600),'The flood keeps rising with the row past row 28');
  assert.equal(tide(unDriven,28,1800),tide(unDriven,28,400),'The flat flood settles as it always did');
  const band=(list,lo,hi,get)=>{const hit=list.filter(x=>x.row>lo&&x.row<=hi);return hit.length?hit.reduce((s,x)=>s+get(x),0)/hit.length:NaN;};
  // Each channel read over a band of rows either side of 200 and of 500, deep in one chart.
  for(const [name,list,get] of [['hazard radius',driven.hazards.filter(h=>h.kind!=='wind'),h=>h.r],['field strength',driven.hazards,h=>h.force]]){
    const a=band(list,20,28,get),b=band(list,180,220,get),c=band(list,480,520,get);
    assert(c>b&&b>a,'The '+name+' keeps rising at rows 200 and 500: '+[a,b,c].map(v=>v.toFixed(2)).join(' → '));
  }
  const winds=(w,lo,hi)=>w.hazards.filter(h=>h.kind==='wind'&&h.row>lo&&h.row<=hi).length;
  assert(winds(driven,100,300)>winds(flat,100,300),'Past the driver\'s first unit a deep chart carries more wind-heads than the flat one');
  const lethal=(w,lo,hi)=>w.hazards.filter(h=>h.kind!=='wind'&&h.row>lo&&h.row<=hi).length;
  assert(lethal(driven,320,560)>lethal(flat,320,560),'Past the driver\'s third unit a hazard may stand on every row');
  assert(Math.max(...driven.nebulas.filter(g=>g.row>400).map(g=>g.r))>90,'Past the old cap a deep chart may hang a larger cloud');
  // The field strength is read by the flight and the guide alike: the same hazard pulls harder when it is cut stronger.
  const pull=force=>{const p={x:0,y:-110,vx:200,vy:0};bendVelocity(p,[{x:0,y:0,r:24,kind:'vortex',force}],1/120);return Math.atan2(p.vy,p.vx);};
  assert(pull(1.4)>pull(1)&&pull(1)>0,'A stronger field turns a flight harder, and an unmarked one turns it as it always did');
}
if(!quick){
