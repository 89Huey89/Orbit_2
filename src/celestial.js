...
function drawAtmosphere(dt=0,aim=null){
  ...
  if(W>780&&!plainPlate()){
    ...
    // Both glosses are set on one line across the sheet. MOMENTUM used to stand nearly a quarter of a
    // plate lower, which is the ground the MAGNITUDINES key now stands on above the scale bar (see
    // buildFrameLayer's wide flank in frame.js); on a short wide window the two headings ran into each
    // other's underline. A marginal note answering another marginal note straight across the play
    // channel is also what a printed sheet does with a pair of glosses.
    const glossY=H*.45;
    ...
    ctx.textAlign='right';ctx.fillText('MOMENTUM',W*.88,glossY);line(W*.88-34,glossY+12,W*.88,glossY+12,`rgba(${ink.atmosphere.annotation},.16)`);
  }
